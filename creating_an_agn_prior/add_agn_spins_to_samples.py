...
def add_agn_samples_to_df(df):
    ...
    df['s1_dot_s2'] = (df['s1x'] * df['s2x']) + (df['s1y'] * df['s2y']) + (df['s1z'] * df['s2z'])
    df['s1_mag'] = np.sqrt((df['s1x'] * df['s1x']) + (df['s1y'] * df['s1y']) + (df['s1z'] * df['s1z']))
    df['s2_mag'] = np.sqrt((df['s2x'] * df['s2x']) + (df['s2y'] * df['s2y']) + (df['s2z'] * df['s2z']))
    df['cos_theta_12'] = df['s1_dot_s2'] / (df['s1_mag'] * df['s2_mag'])
    # Lhat = [0, 0, 1]
    df['cos_theta_1L'] = df['s1z'] / (df['s1_mag'])
    print(df['cos_theta_1L'].describe())
    return df
...
def process_res(r):
    ...
    df['tilt1'], df['theta_1L'] = np.arccos(df['cos_tilt_1']), np.arccos(df['cos_theta_1L'])
